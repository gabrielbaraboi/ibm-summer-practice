...
const getUserProfilePic = async (req, res) => {
	...
	try {
		...
		{
			return res.status(400).json({message: "user id wasn't found in database"});
		}
	} catch (err) {
		return res.status(404).json({ message: err.message });
	}
...
const getUserDetails = async (req, res) => {
	...
	try {
		...
		if(user)
			...
			return res.status(400).json({message : "user id wasn't found in the database"});
	} catch (err) {
		return res.status(404).json({ message: err.message });
	}
};
const setProfilePic = async (req,res) => {
	try {
		...
		{
			if(user.profilePic)
				await ibm.deletePic(user.profilePic);
			...
			return res.status(404).json({ message: "user id wasn't found in the database" });
	} catch (e) {
		console.log(e);
...
module.exports = { getUserProfilePic, getUserDetails, setProfilePic, modifySocialMedia, modifyAboutMe };
